...
def test_build_frozen_tool_environment_includes_config_path() -> None:
    environment = module.build_frozen_tool_environment()
    pythonpath_entries = environment["PYTHONPATH"].split(os.pathsep)

    assert str(module.FROZEN_CONFIG_ROOT) in pythonpath_entries
    assert str(module.FROZEN_STICKS_ROOT) in pythonpath_entries
...
def test_create_curated_operations_uses_operation_cli(monkeypatch) -> None:
    ...
    def fake_create_operation(
        ...
        calls.append((operation_name, group_name, adversary_id))
