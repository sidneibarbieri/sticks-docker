...
ENTERPRISE_BUNDLE = WORKSPACE_ROOT / "sticks" / "data" / "stix" / "enterprise-attack.json"
...
def write_tex(field_stats: dict, itemsets: list[dict], docker_rows: list[dict], total_attack_patterns: int) -> None:
    field_rows = [
        ("\\texttt{kill\\_chain\\_phases}", field_stats["kill_chain_phases"]["present"], field_stats["kill_chain_phases"]["non_empty"]),
        ("\\texttt{x\\_mitre\\_platforms}", field_stats["x_mitre_platforms"]["present"], field_stats["x_mitre_platforms"]["non_empty"]),
        ("\\texttt{x\\_mitre\\_system\\_requirements}", field_stats["x_mitre_system_requirements"]["present"], field_stats["x_mitre_system_requirements"]["non_empty"]),
        ("\\texttt{x\\_mitre\\_detection}", field_stats["x_mitre_detection"]["present"], field_stats["x_mitre_detection"]["non_empty"]),
        ("\\texttt{x\\_mitre\\_data\\_sources}", field_stats["x_mitre_data_sources"]["present"], field_stats["x_mitre_data_sources"]["non_empty"]),
        ("\\texttt{x\\_mitre\\_permissions\\_required}", field_stats["x_mitre_permissions_required"]["present"], field_stats["x_mitre_permissions_required"]["non_empty"]),
    ]
    field_body = "\n".join(
        f"{name} & {present} & {non_empty} \\\\"
        for name, present, non_empty in field_rows
    )
    itemset_body = "\n".join(
        f"{row['size']} & {row['max_support']} & {row['fraction_pct']:.1f}\\% \\\\"
        for row in itemsets
    )
    docker_body = "\n".join(
    ...
    TEX_VALUES.write_text(
        "\n".join(
            [
                f"\\newcommand{{\\nAppendixAttackPatterns}}{{{total_attack_patterns}}}",
...
def main() -> None:
    ...
    write_tex(field_stats, itemsets, docker_rows, len(attack_patterns))
    write_provenance(field_stats, itemsets, docker_rows, len(attack_patterns))
    print(f"Wrote {TEX_VALUES}")
