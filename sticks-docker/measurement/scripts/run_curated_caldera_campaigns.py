...
FROZEN_CONFIG_ROOT = FROZEN_STICKS_ROOT / "config"
...
def build_frozen_tool_environment() -> dict[str, str]:
    environment = os.environ.copy()
    pythonpath_entries = [str(FROZEN_CONFIG_ROOT), str(FROZEN_STICKS_ROOT)]
...
def run_frozen_tool(arguments: list[str]) -> subprocess.CompletedProcess[str]:
    completed = subprocess.run(
        arguments,
        cwd=FROZEN_STICKS_ROOT,
        capture_output=True,
        text=True,
        check=False,
        env=build_frozen_tool_environment(),
    )
