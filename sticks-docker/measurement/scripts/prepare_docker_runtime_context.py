#!/usr/bin/env python3
"""
Prepare a temporary Docker runtime context for the frozen Paper 1 artifact.
...
def write_results(payload: dict[str, object]) -> tuple[Path, Path]:
    ...
    timestamp = str(payload["generated_at_utc"]).replace(":", "").replace("-", "")
    json_path = RESULTS_DIR / f"docker_runtime_context_{timestamp}.json"
    md_path = RESULTS_DIR / f"DOCKER_RUNTIME_CONTEXT_{timestamp}.md"
